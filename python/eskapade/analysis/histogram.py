...
class BinningUtil(object):
    ...
    def __init__(self, **kwargs):
        ...
        if self.bin_specs:
            ...
            if not any(k in self.bin_specs for k in ['bin_edges', 'bin_width']):
                raise RuntimeError('unable to interpret bin specifications: %s' % self.bin_specs)
    ...
    def truncated_bin_edges(self, variable_range=[]):
        ...
        if ttest or not len(variable_range) == 2 or variable_range[1] <= variable_range[0]:
            raise RuntimeError('expected a variable range (min, max); got "{}"'.format(str(variable_range)))
        if 'bin_edges' in self.bin_specs:
            bin_edges = self.bin_specs['bin_edges']
            if variable_range[0] > bin_edges[-1] or variable_range[1] <= bin_edges[0]:
                raise RuntimeError('expected edges in range ({0:f}, {1:f}); got "{2:s}"'.format(bins[0], bins[-1],
                                                                                                str(variable_range)))
        ...
        if 'bin_edges' in self.bin_specs:
            ...
            # the previous bin's right edge.
            max_idx = self.value_to_bin_label(variable_range[1], greater_equal=True)
            bins = variable_range = (self.get_left_bin_edge(min_idx), self.get_right_bin_edge(max_idx))
...
class Histogram(BinningUtil, ArgumentsMixin, LoggingMixin):
        ...
        * ValueCounts: a ValueCounts object contains a value_counts dictionary.
        ...
        if isinstance(counts, ValueCounts):
            ...
            try:
                ...
                self.log().critical('Invalid type for specified counts: "%s"', type(counts).__name__)
    ...
    def _from_dict(self, counts):
        ...
        if not all(len(k) == 1 for k in counts.keys()):
            self.log().critical('specified counts dictionary contains keys with multiple variable values')

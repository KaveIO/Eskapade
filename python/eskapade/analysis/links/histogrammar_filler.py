        ...
        * add to datastore
        ...
        for c in self.columns:
            for col in c:
                ...
                if col not in colset: colset.append(col)
...
def only_bool(x):
    ...
    """
    if isinstance(x,np.bool_) or isinstance(x,bool): return x
...
def only_int(x):
    ...
    """
    if isinstance(x,np.int64) or isinstance(x,int): return x
    elif hasattr(x,'__iter__') and not isinstance(x,str):
...
def only_float(x):
    ...
    """
    if isinstance(x,np.float64) or isinstance(x,float): return x
    elif hasattr(x,'__iter__') and not isinstance(x,str):

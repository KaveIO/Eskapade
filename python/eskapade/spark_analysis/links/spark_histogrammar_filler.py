...
class SparkHistogrammarFiller(HistogrammarFiller):
    ...
    def __init__(self, **kwargs):
        """Initialize SparkHistogrammarFiller instance
    ...
    def construct_empty_hist(self, df, columns):
        ...
        for col in reversed(columns):
            ...
            if is_number or is_timestamp:
                ...
                hist = histogrammar.SparselyBin(binWidth=bs['bin_width'], origin=bs['bin_offset'], quantity=df[col],
                                                value=hist)
            else:
    ...
    def process_and_store(self):
        ...
        for name, hist in self._hists.items():
            self.log().debug('Processing histogram %s' % name)
    ...
    def reset_hgr_quantity(self, hist, new_quantity = unit_func):
        ...
        if hasattr(hist, 'quantity'):
            #self.log().debug('Reset quantity of obj-type %s' % (type(hist)))
        ...
        try:
            ...
            for i, bin in enumerate(bins):
                self.reset_hgr_quantity(bin, new_quantity)
        except:
            return

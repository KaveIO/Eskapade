...
class UdfTest(unittest.TestCase):
    ...
    def test_timestamp(self, mock_pd):
        ...
        for vals, res in zip(test_vals, test_results):
            ...
            self.assertIs(dt, res.to_datetime() if res else res,
                          'unexpected resulting date/time object for {}'.format(str(vals)))
            if not 'tz_out' in vals:
                ts = to_timestamp(**vals)
                self.assertIs(ts, res.value if res else res, 'unexpected resulting timestamp for {}'.format(str(vals)))
    ...
    def test_calc_asym(self):
        ...
        for vals, exp_res in zip(test_vals, test_results):
            res = calc_asym(*vals)
            if exp_res is None:
                self.assertIs(res, None, 'asymmetry value for {} is not None'.format(str(vals)))
                continue
            self.assertIsInstance(res, float, 'asymmetry value for {} is not a float'.format(str(vals)))
            if exp_res == 'nan':
                self.assertNotEqual(res, res, 'asymmetry value for {} is not NaN'.format(str(vals)))
                continue
            self.assertAlmostEqual(res, exp_res, places=10,
                                   msg='unexpected asymmetry value for {}'.format(str(vals)))
    ...
    def test_spark_sql_func(self, sql_funcs):
        ...
        for vals, res in zip(test_vals, test_results):
            if res is not None:
                ...
                self.assertEqual(func, res, 'unexpected function returned for {}'.format(str(vals)))
            else:

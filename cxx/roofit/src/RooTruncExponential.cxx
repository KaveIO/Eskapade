...
Double_t RooTruncExponential::evaluate() const
{
  ...
  if (!_ignoreMax && _var > _maxVar) return 0.;
    ...
    const char* rangeName) const
{
  ...
  if (!_ignoreMax) varMax = std::min(varMax, (Double_t)_maxVar);
...
{
  if (matchArgs(directVars, generateVars, _var)) return 1;
  return 0;
}

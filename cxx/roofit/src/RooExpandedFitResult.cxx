...
RooExpandedFitResult::RooExpandedFitResult(const RooFitResult& origResult, const RooArgList& extraPars) :
  RooFitResult(origResult)
{

    TString name,title ;
    name = Form("Expanded%s", this->GetName()) ;
    title = Form("Expanded%s", this->GetTitle()) ;
    ...
    RooArgList saveFloatInitList =  this->floatParsInit();
    RooArgList origFloatInitList =  this->floatParsInit();
    RooArgList saveFloatFinalList = this->floatParsFinal();
    RooArgList origFloatFinalList =  this->floatParsFinal();
    ...
    for (Int_t ic=0; ic<saveFloatFinalList.getSize(); ic++) {
        ...
        if( origFloatFinalList.contains(*par)) globalCC.push_back(this->globalCorr(*par));
    ...
    for (Int_t ic=0; ic<saveFloatFinalList.getSize(); ic++) {
        ...
        for (Int_t ii=0; ii<saveFloatFinalList.getSize(); ii++) {
            ...
            if( origFloatFinalList.contains(*par1) && origFloatFinalList.contains(*par2)){
                corrs(ic,ii) = this->correlation(*par1,*par2);
                covs(ic,ii) = this->covarianceMatrix()[ic][ii];
            }
            else{
...
RooExpandedFitResult::RooExpandedFitResult(const RooArgList& extraPars) :
  RooFitResult()
{

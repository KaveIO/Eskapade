 ...
 *    File: $Id: RhhNDKeysPdf.cxx 31258 2009-11-17 22:41:06Z wouter $
 * Authors:                                                                  *
 *   Max Baak, CERN, mbaak@cern.ch *
 *                                                                           *
 * Description:                                                              *
 *   Includes updates to RooNDKeysPdf that have not yet been 
 *   accepted in official root release.
 *
...
ClassImp(RhhNDKeysPdf)



//_____________________________________________________________________________
RhhNDKeysPdf::RhhNDKeysPdf(const char *name, const char *title,
  ...
  _tracker(0)
{
  ...
  for  (Int_t j=0; j<rho.GetNrows(); j++) { _rho[j]=rho[j]; /*cout<<"RhhNDKeysPdf ctor, _rho["<<j<<"]="<<_rho[j]<<endl;*/ }
  ...
  _nAdpt(1)
{
  ...
  for (Int_t i=0; (rho = (RooAbsArg*)rhoItr->Next()); ++i) {
    if (!dynamic_cast<RooAbsReal*>(rho)) {
      coutE(InputArguments) << "RhhNDKeysPdf::ctor(" << GetName() << ") ERROR: parameter " << rho->GetName() 
  ...
  if( (_varList.getSize() != _rhoList.getSize()) ) {
    coutE(InputArguments) << "ERROR:  RhhNDKeysPdf::RhhNDKeysPdf() : The size of rhoList is different from varList."
			  << "Unable to create the PDF." << endl;
...
{ 
  ...
  if (mirror!=NoMirror) {
    if (mirror!=MirrorBoth) 
      coutW(InputArguments) << "RhhNDKeysPdf::RhhNDKeysPdf() : Warning : asymmetric mirror(s) no longer supported." << endl;
    _options="m";
  }
...
//_____________________________________________________________________________
RhhNDKeysPdf::RhhNDKeysPdf(const char *name, const char *title, RooAbsReal& x, RooAbsReal & y,
...
//_____________________________________________________________________________
RhhNDKeysPdf::RhhNDKeysPdf(const RhhNDKeysPdf& other, const char* name) :
...
void
//_____________________________________________________________________________
RhhNDKeysPdf::createPdf(Bool_t firstCall) const
{
...
RhhNDKeysPdf::setOptions() const
{
  ...
  cxcoutD(InputArguments) << "RhhNDKeysPdf::setOptions()    options = " << _options 
  ...
  if (_nSigma<2.0) {
    coutW(InputArguments) << "RhhNDKeysPdf::setOptions() : Warning : nSigma = " << _nSigma << " < 2.0. "
...
RhhNDKeysPdf::initialize() const
{
  ...
  if(_nDim==0) {
    coutE(InputArguments) << "ERROR:  RhhNDKeysPdf::initialize() : The observable list is empty. "
			  << "Unable to begin generating the PDF." << endl;
    assert (_nDim!=0);
  }

  if(_nEvents==0) {
    coutE(InputArguments) << "ERROR:  RhhNDKeysPdf::initialize() : The input data set is empty. "
			  << "Unable to begin generating the PDF." << endl;
    assert (_nEvents!=0);
  }
...
//_____________________________________________________________________________
RhhNDKeysPdf::loadDataSet(Bool_t firstCall) const
{
  ...
  coutI(Contents) << "RhhNDKeysPdf::loadDataSet(" << this << ")" 
...
//_____________________________________________________________________________
RhhNDKeysPdf::mirrorDataSet() const
{
...
void
//_____________________________________________________________________________
RhhNDKeysPdf::loadWeightSet() const
{
  _wMap.clear();
  ...
  coutI(Contents) << "RhhNDKeysPdf::loadWeightSet(" << this << ") : Number of weighted events : " << _wMap.size() << endl;
}


void
//_____________________________________________________________________________
RhhNDKeysPdf::calculateShell(BoxInfo* bi) const 
{
  ...
  coutI(Contents) << "RhhNDKeysPdf::calculateShell() : " 
...
void
//_____________________________________________________________________________
RhhNDKeysPdf::calculatePreNorm(BoxInfo* bi) const
{
  //bi->nEventsBMSW=0.;
  //bi->nEventsBW=0.;
  ...
  cxcoutD(Eval) << "RhhNDKeysPdf::calculatePreNorm() : " 
...
void
//_____________________________________________________________________________
RhhNDKeysPdf::sortDataIndices(BoxInfo* bi) const
{
  ...
  for (Int_t j=0; j<_nDim; j++) { 
    cxcoutD(Eval) << "RhhNDKeysPdf::sortDataIndices() : Number of sorted events : " << _sortTVIdcs[j].size() << endl; 
  }
}
...
void
//_____________________________________________________________________________
RhhNDKeysPdf::calculateBandWidth() const
{
  cxcoutD(Eval) << "RhhNDKeysPdf::calculateBandWidth()" << endl; 

  // non-adaptive bandwidth 
  // (default, and needed to calculate adaptive bandwidth)

  if(!_options.Contains("a")) {
      cxcoutD(Eval) << "RhhNDKeysPdf::calculateBandWidth() Using static bandwidth." << endl;
  }    
  ...
  // adaptive width
  if( _options.Contains("a") ) {
    cxcoutD(Eval) << "RhhNDKeysPdf::calculateBandWidth() Using adaptive bandwidth." << endl;
...
Double_t
//_____________________________________________________________________________
RhhNDKeysPdf::gauss(vector<Double_t>& x, vector<vector<Double_t> >& weights) const 
{
  ...
  for (; ibMapItr!=ibMapEnd; ++ibMapItr) {
    ...
    // skip empty bins in gauss evaluation
    if ( _wMap[_idx[i]]==0 ) continue;
...
void
//_____________________________________________________________________________
RhhNDKeysPdf::loopRange(vector<Double_t>& x, map<Int_t,Bool_t>& ibMap) const
{
  ibMap.clear();
...
void
//_____________________________________________________________________________
RhhNDKeysPdf::boxInfoInit(BoxInfo* bi, const char* rangeName, Int_t /*code*/) const
{
...
Int_t 
//_____________________________________________________________________________
RhhNDKeysPdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const
{
...
Double_t 
//_____________________________________________________________________________
RhhNDKeysPdf::analyticalIntegral(Int_t code, const char* rangeName) const
{
  cxcoutD(Eval) << "Calling RhhNDKeysPdf::analyticalIntegral(" << GetName() << ") with code " << code 
  ...
  // reset
  if (newBounds) {
    cxcoutD(Eval) << "RhhNDKeysPdf::analyticalIntegral() : Found new boundaries ... " << (rangeName?rangeName:"<none>") << endl;
    boxInfoInit(bi,rangeName,code);    
  }
  ...
  if (_mirror && bi->netFluxZ) {
    ...
    cxcoutD(Eval) << "RhhNDKeysPdf::analyticalIntegral() : Using mirrored normalization : " << bi->nEventsBW << endl;
    return bi->nEventsBW;
  } 
  ...
  else 
  {
    ...
    cxcoutD(Eval) << "RhhNDKeysPdf::analyticalIntegral() : Final normalization : " << norm << " " << bi->nEventsBW << endl;
    return norm;
  }
}
...
RooDataSet* 
//_____________________________________________________________________________
RhhNDKeysPdf::createDatasetFromHist( const RooArgList& varList, const TH1& hist ) const
{
  ...
  for (Int_t i=0; (var = (RooAbsArg*)varItr->Next()); ++i) {
    if (!dynamic_cast<RooRealVar*>(var)) {
      coutE(InputArguments) << "RhhNDKeysPdf::createDatasetFromHist(" << GetName() << ") WARNING: variable " << var->GetName() 
  ...
  if ( histndim>3 || histndim<=0 ) {
    coutE(InputArguments) << "RhhNDKeysPdf::createDatasetFromHist(" << GetName() 
...
TMatrixD 
//_____________________________________________________________________________
RhhNDKeysPdf::getWeights( const int& k ) const
{
  TMatrixD mref( _nEvents, _nDim+1 );

  cxcoutD(Eval) << "RhhNDKeysPdf::getWeights() Return evaluated weights." << endl;

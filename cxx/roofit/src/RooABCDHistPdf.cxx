...
{
  if (_allBCDvec!=0) { delete _allBCDvec; _allBCDvec=0; }
}
...
{
  if (!_abcd) return -999;
  if (!(ibin>=0 && ibin<_dh.numEntries())) return -999;
...
RooABCDHistPdf::constructExpectationFunctions()
{
  ...
  for (Int_t i=0; i<_dh.numEntries(); i++) {
    ...
    if (i%100 == 0)
      coutI(InputArguments) << "RooABCDHistPdf::constructExpectationFunctions(" << GetName() << ") INFO: processing bin " << i << endl ;
    ...
    RooArgList dList;
    for (Int_t j=0; j<_ndim-1; ++j) { dList.add(*sumD_i); }
...
Bool_t Eskapade::ABCD::checkInputData(const RooDataHist& data, Bool_t abcd)
{
  ...
  for (Int_t j=0; (var = (RooAbsArg*)varItr->Next()); ++j) {
    if (dynamic_cast<RooRealVar*>(var)) { continue; }
    if (dynamic_cast<RooCategory*>(var)) { continue; }

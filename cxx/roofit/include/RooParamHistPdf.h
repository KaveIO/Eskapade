...
public:
  ...
  const RooDataHist& getInitialData() const { return _dh; }
  const RooDataHist& getModifiedData() const;

  const RooArgList& paramList() const { return _p ; }
  const RooArgList& nomList() const { return _c ; }
  const RooArgList& binList() const { return _b ; }
  ...
  Int_t getIndex(const RooArgSet& coord, Bool_t fast = kFALSE) { return _dh_mod->getIndex(coord,fast); }
...
protected:
  ...
  public:
    ...
    virtual ~FastHist() {};
    void setWeight(Int_t i, Double_t w) { _wgt[i]=w; }
    void setSumW2(Int_t i, Double_t sw2) { _sumw2[i]=sw2; }
  };

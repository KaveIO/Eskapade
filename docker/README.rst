...
.. code:: bash
  ...
  $  docker pull kave/eskapade-env:0.8.4 

This will download the ``kave/eskapade-env:0.8.4`` image locally.
...
This will produce the ``kave/eskapade-env:0.8.4`` image locally.
...
.. code:: bash
  ...
  $  docker run -p 8888:8888 -it kave/eskapade-env:0.8.4 
...
.. code:: bash
  ...
  $  docker run -v <ESKAPADE>:/opt/eskapade -p 8888:8888 -it kave/eskapade-env:0.8.4 
...
.. code:: bash
  ...
  $  cd docker/eskapade-usr && docker build -t kave/eskapade-usr:0.8.4 .

This will produce the ``kave/eskapade-usr:0.8.4`` image.

From this image, containers with the Eskapade environment set up, can be run out-of-the-box:

.. code:: bash

  $ docker run -e HOST_USER_ID=$(id -u) -e HOST_USER_GID=$(id -g) -p 8888:8888 -it kave/eskapade-usr:0.8.4
...
Copy the top CONTAINER-ID string, matching the running instance of the ``kave/eskapade-usr:0.8.4`` image, and then paste it:

.. code:: bash

  $ docker commit CONTAINER-ID kave/eskapade-usr:0.8.4
...
.. code:: bash
  ...
  $  docker run -e HOST_USER_ID=$(id -u) -e HOST_USER_GID=$(id -g) -v <ESKAPADE>:/home/esdev/eskapade -p 8888:8888 -it kave/eskapade-usr:0.8.4
...
.. code-block:: bash
  ...
  alias eskapade_docker='docker run -e HOST_USER_ID=$(id -u) -e HOST_USER_GID=$(id -g) -v <ESKAPADE>:/home/esdev/eskapade -p 8888:8888 -it kave/eskapade-usr:0.8.4'
